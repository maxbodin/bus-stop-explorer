...
export default function FilterPanel( { filters, isOpen, onToggle }: FilterPanelProps ) {
    ...
    return (
        <div
            className="absolute right-2.5 top-2.5 z-1000 max-h-[80vh] w-[320px] overflow-y-auto rounded-lg border border-neutral-300 bg-white p-4 shadow-lg">
            ...
            { filters.availableRoutes.length > 0 && (
                <div className="mb-2">
                    <div className="mb-2 flex items-center justify-between">
                        <label className="text-xs font-semibold uppercase tracking-wide text-neutral-600">Lignes de
                            bus</label>
                    </div>

                    <div
                        className="routes-scroll flex max-h-75 flex-col gap-2 overflow-y-scroll pr-1 [scrollbar-gutter:stable]">
                        { filters.availableRoutes.map( route => (
                            <label key={ route }
                                   className="flex items-center gap-2 rounded-md p-2 text-sm text-neutral-800 transition hover:bg-neutral-50">
                                <input
                                    type="checkbox"
                                    className="h-4 w-4 cursor-pointer"
